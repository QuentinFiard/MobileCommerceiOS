...
#import "ATGMagnifyingGridCollectionView.h"
#import "ATGGridCollectionViewLayout.h"

#pragma mark - ATGMagnifyingGridCollectionViewLayout Definition
#pragma mark -

@interface ATGMagnifyingGridCollectionViewLayout : ATGGridCollectionViewLayout

- (CGSize)actualItemSize;

@end

#pragma mark - ATGMagnifyingGridCollectionView Private Protocol Definition
#pragma mark -

@interface ATGMagnifyingGridCollectionView () <UICollectionViewDelegate>

@end

#pragma mark - ATGMagnifyingGridCollectionView Implementation
#pragma mark -

@implementation ATGMagnifyingGridCollectionView

#pragma mark - ATGGridCollectionView

+ (Class)gridCollectionViewLayoutClass {
  return [ATGMagnifyingGridCollectionViewLayout class];
}
...
- (void)layoutSubviews {
  ...
  if (![self isDragging] && ![self isDecelerating]) {
    offset = [[self collectionViewLayout] targetContentOffsetForProposedContentOffset:offset
                                                                withScrollingVelocity:CGPointZero];
    [self setContentOffset:offset];
  }
  offset.x += [self bounds].size.width / 2;
  offset.y = [self bounds].size.height / 2;
  NSIndexPath *indexPath = [self indexPathForItemAtPoint:offset];
  if (indexPath) {
    [[self gridViewDelegate] gridCollectionView:self
                               didMagnifyObject:[[self objectsToDisplay] objectAtIndex:[indexPath item]]];
  }
}

@end

#pragma mark - ATGMagnifyingGridCollectionViewLayout Implementation
#pragma mark -

@implementation ATGMagnifyingGridCollectionViewLayout
