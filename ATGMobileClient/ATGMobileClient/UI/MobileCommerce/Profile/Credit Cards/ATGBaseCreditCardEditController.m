...
#import "ATGBaseCreditCardEditController.h"

@interface ATGBaseCreditCardEditController () <UITextFieldDelegate, ATGDatePickerDelegate>
...
@implementation  ATGBaseCreditCardEditController {
}
...
- (void) viewDidLoad {
  ...
  [self cardNicknameInput].delegate = self;
...
#pragma mark - ATGDatePickerDelegate method
...
- (void) didSubmitDone {
  ...
  [[self creditCard] setBillingAddress:[self.addressSection address]];
...
-(void)errorUpdatingCard:(NSError *)error{
  [self stopActivityIndication];
  [self tableView:[self tableView] setError:error inSection:0];
}
...
-(void)didErrorDeletingCard:(NSError *)error{
  [[ATGActionBlocker sharedModalBlocker] dismissBlockView];
  [self alertWithTitleOrNil:nil withMessageOrNil:[error localizedDescription]];
}
