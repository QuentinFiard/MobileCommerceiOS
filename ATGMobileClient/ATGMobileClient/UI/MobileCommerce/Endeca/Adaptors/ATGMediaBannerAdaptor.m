...
#import "ATGMediaBannerAdaptor.h"
...
@interface ATGMediaBannerAdaptor ()
@property (nonatomic, strong) EMMediaBanner *contentItem;
@end

@implementation ATGMediaBannerAdaptor
...
- (BOOL)shouldHighlightItemAtIndex:(NSInteger)pIndex {
  return (self.contentItem.link ? YES : NO);
}

- (void)didSelectItemAtIndex:(NSInteger)pIndex {
  if (self.contentItem.link) {
    [(ATGSearchViewController *)self.controller loadPageForAction:(EMAction *)self.contentItem.link];
  }
}
