...
#import "ATGHorizontalResultsListAdaptor.h"
#import "ATGHorizontalResultsList.h"
#import "ATGHorizontalResultsListRenderer.h"
#import "ATGHorizontalResultsListCarouselCell.h"
...
@interface ATGHorizontalResultsListAdaptor () <ATGCarouselDataSource, ATGCarouselDelegate>
@property(nonatomic, strong) ATGHorizontalResultsList *contentItem;
@property(nonatomic, strong) ATGSearchViewController *controller;
@property(nonatomic, strong) NSString *resultsState;
@end

@implementation ATGHorizontalResultsListAdaptor
...
- (id)objectToBeRenderedAtIndex:(NSInteger)pIndex {
  ...
  return [NSDictionary dictionaryWithObjectsAndKeys:(firstItem ? firstItem : [NSNumber numberWithInt:0]), @"firstItem", [NSNumber numberWithInt:[self.controller.results resultsForState:self.resultsState].count ], @"totalItems",  nil];
}

- (Class)rendererClassForIndex:(NSInteger)pIndex {
  return [ATGHorizontalResultsListRenderer class];
}
...
- (void)usingRenderer:(EMContentItemRenderer *)pRenderer forIndex:(NSInteger)pIndex {
  if ([pRenderer isKindOfClass:[ATGHorizontalResultsListRenderer class]]) {
    ATGHorizontalResultsListRenderer *renderer = (ATGHorizontalResultsListRenderer *)pRenderer;
    [renderer.carousel registerClass:[ATGHorizontalResultsListCarouselCell class] forCellWithReuseIdentifier:@"Cell"];
...
- (void)requestNextBatchOfContentItemsOnCondition:(NSInteger)pIndex {
  NSInteger lastItem = [self.controller.results resultsForState:self.resultsState].count - 4;
  EMNavigationAction *pagingAction = [self pagingActionForNext];
  if (pIndex == lastItem && pIndex + 3 != [self.contentItem.totalNumRecs intValue] - 1) {
    NSDictionary *adaptorAttributes = [NSDictionary dictionaryWithObject:[NSNumber numberWithInt:pIndex + 4] forKey:SINGLE_RESULTS_LIST_OFFSET_KEY];
...
- (UICollectionViewCell *)carousel:(ATGCarouselView *)pCarousel cellForItemAtIndex:(NSInteger)pIndex {
  ATGHorizontalResultsListCarouselCell *cell = (ATGHorizontalResultsListCarouselCell *)[pCarousel dequeueReusableCellWithReuseIdentifier:@"Cell" forIndex:pIndex];
  
  EMRecord *record = (EMRecord *)[self.controller.results recordForState:self.resultsState atIndex:pIndex];
  [cell setObject:(id<RenderableProduct>)record];
  
  [self requestNextBatchOfContentItemsOnCondition:pIndex];
