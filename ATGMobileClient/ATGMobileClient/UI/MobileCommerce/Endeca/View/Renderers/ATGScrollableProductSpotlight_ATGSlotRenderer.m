...
@implementation ATGScrollableProductSpotlight_ATGSlotRenderer

- (CGSize) carouselSize:(CGRect)frame {
...
- (CGSize) cellSize {
  ...
  if ([UIDevice isPhone])
    return CGSizeMake(frameSize.width * 0.27, 140);

  return CGSizeMake(frameSize.width * 0.18, 180);
}

- (void)layoutSubviews {
  CGSize carouselSize = [self carouselSize:self.frame];
  self.carousel.frame = CGRectMake(0, 0, carouselSize.width, carouselSize.height);
  self.carousel.itemSize = [self cellSize];
  self.carousel.pageSize = carouselSize;
