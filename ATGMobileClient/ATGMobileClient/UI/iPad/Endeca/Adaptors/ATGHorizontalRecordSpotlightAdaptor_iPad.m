...
#import "ATGHorizontalRecordSpotlightAdaptor_iPad.h"
#import "ATGBaseBrowseSearchViewController_iPad.h"
#import "ATGResults.h"
#import <EMMobileClient/EMHorizontalRecordSpotlight.h>
#import "ATGHorizontalRecordSpotlightRenderer_iPad.h"
#import "ATGFeaturedProductSpotlightCarouselCell_iPad.h"
#import <ATGUIElements/ATGCarouselView.h>

@class ATGCarouselView;

@interface ATGHorizontalRecordSpotlightAdaptor_iPad () <ATGProductDetailsStackDataSource, ATGCarouselDelegate, ATGCarouselDataSource>
...
@implementation ATGHorizontalRecordSpotlightAdaptor_iPad

- (void)carousel:(ATGCarouselView *)pCarousel didSelectItemAtIndex:(NSInteger)pIndex isActiveItem:(BOOL)pActiveItem {
  EMRecord<RenderableProduct> *record = [self.contentItem.records objectAtIndex:pIndex];
  [self.controller loadDetailsForProduct:record withDataSource:self];
}

#pragma mark - ATGProductDetailsStackDataSource
- (NSArray *) initialProducts {
  return self.contentItem.records;
}
...
- (CGSize)sizeForRendererAtIndex:(NSInteger)pIndex {
  return CGSizeMake(self.controller.view.bounds.size.width, 200);
}

- (Class)rendererClassForIndex:(NSInteger)pIndex{
  return [ATGHorizontalRecordSpotlightRenderer_iPad class];
}

- (void)usingRenderer:(EMContentItemRenderer *)pRenderer forIndex:(NSInteger)pIndex {
  ATGHorizontalRecordSpotlightRenderer_iPad *renderer = (ATGHorizontalRecordSpotlightRenderer_iPad *)pRenderer;
  [renderer.carousel registerClass:[ATGFeaturedProductSpotlightCarouselCell_iPad class] forCellWithReuseIdentifier:@"Cell"];
...
- (UICollectionViewCell *)carousel:(ATGCarouselView *)pCarousel cellForItemAtIndex:(NSInteger)pIndex {
  ATGFeaturedProductSpotlightCarouselCell_iPad *cell = (ATGFeaturedProductSpotlightCarouselCell_iPad *)[pCarousel dequeueReusableCellWithReuseIdentifier:@"Cell" forIndex:pIndex];
