...
#import "ATGProductList_ATGCategoryChildrenAdaptor_iPad.h"
...
@interface ATGProductList_ATGCategoryChildrenAdaptor_iPad () <ATGProductDetailsStackDataSource>
@property (nonatomic, strong) ATGProductList_ATGCategoryChildren *contentItem;

@end

@implementation ATGProductList_ATGCategoryChildrenAdaptor_iPad
...
- (void)didSelectItemAtIndex:(NSInteger)pIndex {
  if (pIndex < self.contentItem.atgContents.childProducts.count) {
    ...
    [((ATGBaseBrowseSearchViewController_iPad *)self.controller) loadDetailsForProduct:(id<RenderableProduct>)product withDataSource:self];
  }
}

- (NSArray *) initialProducts {
  return self.contentItem.atgContents.childProducts;
}
...
- (Class)rendererClassForIndex:(NSInteger)pIndex {
  if (pIndex < self.contentItem.atgContents.childProducts.count) {
    return [ATGProductList_ATGCategoryChildrenRenderer_iPad class];
  }
