...
@implementation ATGCheckoutDefaultsShippingMethodViewController

- (void) viewDidLoad {
  [super viewDidLoad];
  NSString *title = NSLocalizedStringWithDefaultValue(
    @"ATGCheckoutDefaultsShippingMethodViewController.Title", nil,
    [NSBundle mainBundle], @"Shipping Method", @"Screen title to be displayed.");
  [self setTitle:title];
...
- (NSInteger) numberOfSectionsInTableView:(UITableView *)pTableView {
  // Display all methods in a single section.
  return 1;
}
...
- (UITableViewCell *) tableView:(UITableView *)pTableView
  ...
  UITableViewCell *cell = [pTableView dequeueReusableCellWithIdentifier:@"cell"];
  if (!cell) {
    // Always use standard cells.
    cell = [[ATGShippingMethodTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
...
- (void) tableView:(UITableView *)pTableView willDisplayCell:(UITableViewCell *)pCell
 forRowAtIndexPath:(NSIndexPath *)pIndexPath {
  // We're about to display the cell, update it with proper method.
  [pCell setAccessoryType:UITableViewCellAccessoryDisclosureIndicator];
  [(ATGShippingMethodTableViewCell *) pCell
   setShippingMethod:[[self shippingMethods] objectAtIndex:[pIndexPath row]]];
...
- (void) tableView:(UITableView *)pTableView didSelectRowAtIndexPath:
 (NSIndexPath *)pIndexPath {
  [[self delegate] didSelectShippingMethod:[[self shippingMethods] objectAtIndex:[pIndexPath row]]];
