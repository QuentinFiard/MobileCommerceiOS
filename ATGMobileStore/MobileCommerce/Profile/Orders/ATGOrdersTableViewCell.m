...
@interface ATGOrdersTableViewCell ()

#pragma mark - IB Outlets

@property (nonatomic, readwrite, weak) IBOutlet ATGImageView *productImageView;
@property (nonatomic, readwrite, weak) IBOutlet UILabel *orderIDLabel;
@property (nonatomic, readwrite, weak) IBOutlet UILabel *orderIDCaptionLabel;
...
#pragma mark - ATGOrdersTableViewCell Implementation
#pragma mark -

@implementation ATGOrdersTableViewCell

#pragma mark - Synthesized Properties

@synthesize productImageView;
@synthesize orderIDLabel;
@synthesize orderIDCaptionLabel;
@synthesize itemsCaptionLabel;
@synthesize dateCaptionLabel;
@synthesize statusCaptionLabel;
...
- (void)awakeFromNib {
  ...
  // apply styles
  [self.orderIDLabel applyStyleWithName:@"formFieldBlackLabel"];
  [self.orderIDCaptionLabel applyStyleWithName:@"formFieldLabel"];
  ...
  NSString *caption = NSLocalizedStringWithDefaultValue
      (@"ATGOrdersTableViewCell.ItemsCaption", nil, [NSBundle mainBundle], @"Items:",
       @"'Items:' caption to be used on the orders cell.");
  ...
  caption = NSLocalizedStringWithDefaultValue
      (@"ATGOrdersTableViewCell.DatePlacedCaption", nil, [NSBundle mainBundle],
       @"Order Placed:", @"'Order Placed:' caption to be used on the orders cell.");
  ...
  caption = NSLocalizedStringWithDefaultValue
  (@"ATGOrdersTableViewCell.SiteCaption", nil, [NSBundle mainBundle], @"Site:",
   @"'Site:' caption to be used on the orders cell.");
  ...
  caption = NSLocalizedStringWithDefaultValue
      (@"ATGOrdersTableViewCell.StatusCaption", nil, [NSBundle mainBundle], @"Status:",
       @"'Status:' caption to be used on the orders cell.");
  ...
  caption = NSLocalizedStringWithDefaultValue
  (@"ATGOrdersTableViewCell.OrderIDCaption", nil, [NSBundle mainBundle], @"Order #:",
   @"'Order #:' caption to be used on the orders cell.");
  [[self orderIDCaptionLabel] setText:caption];
  self.orderIDCaptionLabel.frame = [self.orderIDCaptionLabel textRectForBounds:self.orderIDCaptionLabel.frame limitedToNumberOfLines:1];
  ...
  UIView *underlay = [[UIView alloc] initWithFrame:CGRectMake(5, 8, 80, 80)];
  ...
  imageOverlay.frame = CGRectMake(5, 8, 80, 80);
...
- (void)setObject:(id)object {
  ...
  [[self orderIDLabel] setText:[order orderId]];
  self.orderIDLabel.left = self.orderIDCaptionLabel.right + INTERLABEL_PAD;
  [[self itemsLabel] setText:[[self numberFormatter]
                              stringFromNumber:[order totalItems]]];
  self.itemsLabel.left = self.itemsCaptionLabel.right + INTERLABEL_PAD;
  [[self dateLabel] setText:[[self dateFormatter] stringFromDate:[order submittedDate]]];
  self.dateLabel.left = self.dateCaptionLabel.right + INTERLABEL_PAD;
  [[self siteLabel] setText:[order siteName]];
  self.siteLabel.left = self.siteCaptionLabel.right + INTERLABEL_PAD;
  [[self statusLabel] setText:[order status]];
  self.statusLabel.left = self.statusCaptionLabel.right + INTERLABEL_PAD;
  
  [[self productImageView] setImageURL:[ATGRestManager getAbsoluteImageString:[order thumbnailImageUrl]]];
}
