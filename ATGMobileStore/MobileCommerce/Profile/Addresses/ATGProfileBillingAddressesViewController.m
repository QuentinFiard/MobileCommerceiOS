...
#import "ATGProfileBillingAddressesViewController.h"
#import <ATGMobileClient/ATGProfileManagerRequest.h>
#import <ATGMobileClient/ATGExternalProfileManager.h>

#pragma mark - ATGProfileBillingAddressesViewController Private Protocol
#pragma mark -

@interface ATGProfileBillingAddressesViewController () <ATGProfileManagerDelegate>
...
@implementation ATGProfileBillingAddressesViewController
...
#pragma mark - ATGProfileBillingAddressSection

- (void)didSelectAddress {
  // Override this method to actually create a credit card.
  [self startActivityIndication:YES];
  [self setRequest:[[ATGExternalProfileManager profileManager]
                    selectAddressAndCreateCreditCard:[[self selected] nickname] delegate:self]];
}

- (void)editAddressWithUserStatus:(BOOL)anonymous {
  self.userAnonymous = anonymous;
  [self performSegueWithIdentifier:ATGSegueIdBillingAddressesToProfileBillingAddressEdit sender:self];
}
