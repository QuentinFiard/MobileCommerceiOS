...
#import "ATGProfileBillingAddressSection.h"
...
#pragma mark - ATGProfileBillingAddressSection Implementation
#pragma mark -

@implementation ATGProfileBillingAddressSection
...
- (void)didSubmitDone {
  ...
  if (self.creating) {
    [self setRequest:[[ATGExternalProfileManager profileManager]
                      createAddressAndCreateCreditCard:[self.address copy] delegate:self]];
  } else {
...
- (void) didCreateAddressAndCreateCreditCard:(ATGProfileManagerRequest *)pRequestResults {
  ...
  NSMutableArray *vcs = [self.delegate.navigationController.viewControllers mutableCopy];
  while (![[vcs lastObject] isKindOfClass:[ATGProfileCreditCardsController class]]
         && ![[vcs lastObject] isKindOfClass:[ATGProfileCreditCardsController class]]) {
    [vcs removeLastObject];
  }
  [self.delegate.navigationController setViewControllers:vcs animated:YES];
  [self setRequest:nil];
}
