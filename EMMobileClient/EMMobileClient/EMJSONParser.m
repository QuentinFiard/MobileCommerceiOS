...
- (Class) classForObjectWithAttribute:(NSString *) pAttribute value:(NSString *) pValue {
  ...
  if ([pAttribute isEqualToString:TYPE_KEY]) {
    clazz = [self contentItemClassForType:pValue];
    if (!clazz)
      clazz = [EMContentItem class];
  } else if ([pAttribute isEqualToString: CLASS_KEY]) {
    clazz = [self dataObjectClassForClass:pValue];
    if (!clazz)
...
- (id)parseDictionary:(NSDictionary *)pDictionary {
  // required protocol impl, passed to super (JSONParser).
  return [super parseDictionary:pDictionary usingDelegate:self];
}

- (Class)contentItemClassForType:(NSString *)pType {
  NSString *contentItemClassString = [NSString stringWithFormat:@"%@%@", EM_CLASS_PREFIX, pType];
  return NSClassFromString(contentItemClassString);
}

- (Class)dataObjectClassForClass:(NSString *)pClass {
  NSString *dataObjectClassString = [pClass stringByReplacingOccurrencesOfString:CLASS_PACKAGE withString:EM_CLASS_PREFIX];
  return NSClassFromString(dataObjectClassString);
}
