...
-(void) setUp{
  ...
  self.connection = [EMAssemblerConnection connectionWithHost:@"localhost" port:8080 contextPath:@"assembler" responseFormat:EMAssemblerResponseFormatNone urlBuilder:urlBuilder];
  [self.connection setDefaultHeader:@"Accept" value:@"application/json"];
}
-(void) tearDown{
    self.connection = nil;
}
...
-(void) testBrowse {    
    ...
    [self prepare];  
    [self parseTestData:@"testFormatQueryString"];
    
    [self.connection fetchContent:@"browse" 
                  forSiteRootPath:nil 
                     actionString:@"?format=json" 
