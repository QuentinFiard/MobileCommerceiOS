...
@implementation ATGStackedRestRequestFactory

@synthesize factories = _factories;

+(ATGStackedRestRequestFactory *) factoryWithFactories:(NSArray *)pFactories{
  ATGStackedRestRequestFactory *factory = [[ATGStackedRestRequestFactory alloc] init];
  factory.factories = pFactories;
  return factory;
}
...
- (NSMutableURLRequest *)requestWithHTTPMethod:(ATGHTTPMethod)pMethod path:(NSURL *)pPath parameters:(NSDictionary *)pParameters options:(ATGRestRequestOptions) pOptions{
  return [[self.factories lastObject] requestWithHTTPMethod:pMethod path:pPath parameters:pParameters options:pOptions];
}

- (NSDictionary *)modifyParams:(NSDictionary *)pParameters options:(ATGRestRequestOptions)pOptions{
  NSDictionary *modifyParams = pParameters;
  for(id <ATGRestRequestFactory> factory in self.factories){
    modifyParams = [factory modifyParams:modifyParams options:pOptions];
  }
  return modifyParams;
}

- (NSURL *)modifyRequestURL:(NSURL *)pURL options:(ATGRestRequestOptions) pOptions{
  NSURL *modifyRequestURL = pURL;
  for (id<ATGRestRequestFactory> factory in self.factories){
    modifyRequestURL = [factory modifyRequestURL:modifyRequestURL options:pOptions];
  }
...
- (id <ATGRestOperation>)JSONRequestOperationWithRequest:(NSURLRequest *)pRequest success:(void ( ^ ) ( NSObject <ATGRestOperation> *operation , id responseObject ))pSuccess failure:(void ( ^ ) ( NSObject <ATGRestOperation> *operation , NSError *error ))pFailure{
  return [[self.factories lastObject] JSONRequestOperationWithRequest:pRequest success:pSuccess failure:pFailure];
}
- (id <ATGRestOperation>)HTTPRequestOperationWithRequest:(NSURLRequest *)pRequest success:(void ( ^ ) ( NSObject <ATGRestOperation> *operation , id responseObject ))pSuccess failure:(void ( ^ ) ( NSObject <ATGRestOperation> *operation , NSError *error ))pFailure{
  return [[self.factories lastObject] HTTPRequestOperationWithRequest:pRequest success:pSuccess failure:pFailure];
}

- (void)enqueueRestOperation:(NSObject <ATGRestOperation> *)operation withOptions:(ATGRestRequestOptions)options {
  [[self.factories lastObject] enqueueRestOperation:operation withOptions:options];
}

- (void)enqueueRestOperation:(NSObject <ATGRestOperation> *)operation{
  [[self.factories lastObject] enqueueRestOperation:operation];
}
- (void) setUserAgentString:(NSString *)pUserAgent{
  [[self.factories lastObject] setUserAgentString:pUserAgent];
}

- (void)setValue:(id)pValue forKey:(NSString *)pKey onRequest:(NSMutableURLRequest*)pRequest{
  [[self.factories lastObject] setValue:pValue forKey:pKey onRequest:pRequest];
}

-(void)setStringEncoding:(NSStringEncoding)pStringEncoding{
  [[self.factories lastObject] setStringEncoding:pStringEncoding];
}
-(NSStringEncoding)stringEncoding{
  return [[self.factories lastObject] stringEncoding];
}
